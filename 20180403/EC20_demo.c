...
int EC20_init_run(void)
{
	...
	ret = gpio_export("131");
	if (ret < 0)return ret;
	ret = gpio_set_dir(io_id[0]);
	if (ret < 0)return ret;
	ret = gpio_set_dir(io_id[1]);
	if (ret < 0)return ret;
	ret = gpio_set_value(io_id[1], 0);
	if (ret < 0)return ret;
	sleep(1);
	ret = gpio_set_value(io_id[0], 1);
	if (ret < 0)return ret;
	sleep(1);
	ret = gpio_set_value(io_id[1], 1);
	if (ret < 0)return ret;
	sleep(1);
	ret = gpio_unexport("138");
	if (ret < 0)return ret;
	ret = gpio_unexport("131");
	if (ret < 0)return ret;
	sleep(20);
	system("pppd call quectel-ppp &");
